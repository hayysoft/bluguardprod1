...
def settings_page(request):
	url = 'http://104.43.104.122:5050/alerts/'
...
def Lastest_Device_Data(request):
	...
	for row in devices:
		...
		try:
			...
			Device_Mac_Link = f'http://104.43.104.122:5050/device_json_display/{row[9]}/'
			Is_Link = True
		except IndexError:
			file = None
			Device_Mac_Link = '#'
...
def Quanrantine_Surveillance_Data(request):
	...
	for row in range(len(data)):
		...
		data[row]['Breach_Link'] = f"http://104.43.104.122:5050/invidual_quarantine/{Wearer_ID}/"
		data[row]['Assign_Unassign'] = f"http://104.43.104.122:5050/set_to_assigned_unassigned/{Wearer_ID}/"
		data[row]['set_Q_Device_and_Q_End'] = f"http://104.43.104.122:5050/set_Q_Device_and_Q_End/?Device_ID={Device_ID}&Wearer_ID={Wearer_ID}&Patient_Tag={'Patient_Tag'}"
		data[row]['set_Q_Device_and_Q_Start'] = f"http://104.43.104.122:5050/set_Q_Device_and_Q_Start/?Device_ID={Device_ID}&Wearer_ID={Wearer_ID}&Patient_Tag={'Patient_Tag'}"
...
def communication(request):
	...
	url = 'http://104.43.104.122:5050/get-messages/'
	response = requests.get(url)
...
def Subscription_View(request):
	url = 'http://104.43.104.122:5050/subscriptions/'
	response = requests.get(url)
